...
Supports four operating modes:
  ...
  --mode simulate
      Creates a virtual loopback adapter that the running USB Relay app
      detects (like plugging in a physical device).  The app handles
      adapter detection, NAT/firewall checks, and DHCP — the test only
      acts as the mobile device client.  Requires Administrator
      privileges, the relay app running, and setup_admin.ps1.
...
Usage:
  ...
  python tests/simulate_wmdc_device.py --mode simulate    # virtual adapter (admin)
...
# ---------------------------------------------------------------------------
# Simulate Test (virtual loopback adapter)
# ---------------------------------------------------------------------------
...
def run_simulate_test(
    ...
    """Simulate a USB device connection against the running USB Relay app.

    Creates a Microsoft Loopback Adapter named 'USBRelaySimulated'.
    The running USB Relay application detects the adapter (its
    WMDCMonitor now recognises adapters named ``USBRelay*``), runs
    its full flow (preconfiguration checks, IP verification, DHCP
    server startup), and shows logs in the GUI — exactly as it would
    for a physical RNDIS device.

    The test then acts as the mobile device: performs the DHCP
    handshake against the relay app's server and validates
    connectivity.

    Requires:
      - Windows with Administrator privileges
      - The USB Relay application running with Windows Mobile mode
      - setup_admin.ps1 has been run (NAT + firewall rules)
    """
    print_header("SIMULATE TEST: Full USB Relay Flow")

    # Gate: Windows only
    if sys.platform != 'win32':
        print_fail("Simulate mode requires Windows (uses Microsoft Loopback Adapter)")
        return False

    # Gate: admin privileges
    if not _is_admin():
        print_fail("Simulate mode requires Administrator privileges")
        print_info("Right-click your terminal and select 'Run as administrator',")
        print_info("then re-run: python tests\\simulate_wmdc_device.py --mode simulate")
        return False
    ...
    try:
        # Step 1: Create virtual adapter (simulates USB RNDIS hardware)
        print_section("Creating Virtual Loopback Adapter")
        print_info("This adapter simulates a physical USB RNDIS connection.")
        print_info("The running USB Relay app should detect it automatically.")
        adapter.setup()

        if not adapter.is_ready:
            print_fail("Virtual adapter setup failed")
            return False

        print_pass(f"Adapter '{adapter.adapter_name}' created")
        print_info(f"Interface index: {adapter.interface_index}")
        print_info(f"Gateway IP: {adapter.gateway_ip}")

        # Step 2: Wait for the relay app to detect the adapter and start DHCP
        print_section("Waiting for USB Relay App")
        print_info("The relay app's WMDCMonitor should detect the adapter,")
        print_info("verify NAT/firewall, and start the DHCP server.")
        print_info("Check the relay app's GUI for connection logs.")
        print_info("")

        # The relay app polls every ~2s and needs time for IP verification
        # and DHCP startup.  Try the handshake with retries to give it time.
        max_attempts = 5
        wait_between = 3.0
        lease = None
        ...
        for attempt in range(1, max_attempts + 1):
            print_info(f"Attempt {attempt}/{max_attempts}: "
                       f"sending DHCP DISCOVER...")

            # The relay's DHCP server binds to 0.0.0.0:67.  Target
            # localhost since the loopback adapter cannot broadcast.
            client = DHCPClient(
                server_port=67,
                client_port=68,
                bind_address='',
            ...
            if attempt < max_attempts:
                print_info(f"No response yet — waiting {wait_between:.0f}s "
                           f"for relay app to start DHCP...")
                time.sleep(wait_between)

        # Step 3: Print client log and check result
        print_section("DHCP Client Handshake")
        ...
        if not lease:
            print_fail("DHCP handshake failed — no response from relay app")
            print_info("")
            print_info("Checklist:")
            print_info("  1. Is the USB Relay app running?")
            print_info("  2. Is Windows Mobile mode started (click START)?")
            print_info("  3. Has setup_admin.ps1 been run as admin?")
            print_info("  4. Check the relay app's log for errors.")
            return False

        print_pass("DHCP handshake completed (served by USB Relay app)")

        # Step 4: Validate lease parameters
        print_section("Lease Validation")
        all_ok = True
        ...
        # Step 5: Connectivity test through the relay's NAT
        if not skip_connectivity:
            print_section("End-to-End Connectivity (through USB Relay NAT)")
            ...
            if result.all_passed:
                print_pass("End-to-end connectivity validated through relay!")
            else:
                print_fail("Connectivity validation failed through relay")
                all_ok = False
        ...
        print_info("Removing virtual adapter (relay app should show disconnect)...")
        adapter.teardown()
        if adapter.is_ready:
            print_warn("Adapter cleanup may have failed — check manually")
        else:
            print_pass("Virtual adapter removed")
...
Examples:
  ...
  %(prog)s --mode simulate              Virtual adapter test (admin required)
  ...
  %(prog)s --skip-connectivity          Skip HTTP connectivity tests
    ...
    parser.add_argument(
        '--mode', '-m',
        choices=['unit', 'loopback', 'integration', 'simulate'],
        default='loopback',
