...
.PARAMETER Startup
    Creates an additional scheduled task that re-runs this setup script at
    every system startup.  This ensures the configuration is restored if a
    Windows feature update resets the NAT rule, firewall rule, or scheduled
    task.  The startup task runs as SYSTEM before any user logs in.

.NOTES
    Run from an elevated PowerShell prompt:
        powershell -ExecutionPolicy Bypass -File setup_admin.ps1

    To also enable startup resilience:
...
$StartupTask  = 'USBRelay-StartupRepair'
...
if ($Uninstall) {
    ...
    # Remove startup repair task
    $stask = Get-ScheduledTask -TaskName $StartupTask -ErrorAction SilentlyContinue
    if ($stask) {
        Unregister-ScheduledTask -TaskName $StartupTask -Confirm:$false
        Write-Host "[OK] Startup repair task '$StartupTask' removed." -ForegroundColor Green
    } else {
        Write-Host "[--] Startup repair task '$StartupTask' not found (already removed)." -ForegroundColor Gray
    }
...
# Build the task as raw XML to avoid Duration serialisation bugs in the
# PowerShell ScheduledTask cmdlets (P36500D out-of-range, empty-Duration
# creating a broken/invisible task, etc.).  Omitting <Duration> inside
# <Repetition> is the correct way to express "repeat indefinitely" in the
...
# ---------------------------------------------------------------------------
# Optional: Startup repair task
# ---------------------------------------------------------------------------
if ($Startup) {
    Write-Host "`n[+] Creating startup repair task..." -ForegroundColor White

    # Resolve the full path to this script so the startup task can re-run it
    $scriptPath = $MyInvocation.MyCommand.Path
    if (-not $scriptPath) {
        Write-Host "  WARNING: Could not determine script path. Startup task not created." -ForegroundColor Yellow
        Write-Host "  Re-run with the full path: powershell -File C:\path\to\setup_admin.ps1 -Startup" -ForegroundColor Yellow
    } else {
        # Remove old startup task if it exists
        $oldStartup = Get-ScheduledTask -TaskName $StartupTask -ErrorAction SilentlyContinue
        if ($oldStartup) {
            Unregister-ScheduledTask -TaskName $StartupTask -Confirm:$false
        }
...
"@
        ...
        try {
            Register-ScheduledTask `
                ...
                -Xml $startupXml `
                -ErrorAction Stop | Out-Null
            Write-Host "  Startup repair task '$StartupTask' created." -ForegroundColor Green
            Write-Host "  Configuration will be restored automatically after Windows updates." -ForegroundColor Green
        } catch {
...
if ($Startup) {
    Write-Host "  Startup repair:  $StartupTask (re-runs setup at boot)" -ForegroundColor White
}
Write-Host ""
